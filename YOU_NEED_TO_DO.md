...
- [ ] From launch onward (not before — pre-launch data is recreatable test data): occasionally
  copy a recent `~/thrallo-backups/thrallo-<stamp>/` folder off-host, so a simultaneous VPS +
  Supabase loss cannot take real users' newest backup with it. Details in
