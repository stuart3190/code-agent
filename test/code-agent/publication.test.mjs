...
test("GitHub App runs wait for approval before publishing", async () => {
  ...
  const waiting = await processRun(run, {
    ...
    agentRunner: async ({ context, repositoryMap }) => {
      receivedContext = context;
      receivedMap = repositoryMap;
      return ({
    ...
    repositoryMapRetriever: async () => [{
      id: "symbol-1", name: "fixed", kind: "constant", path: "src/a.js", startLine: 1, references: [],
    }],
  });
  ...
  assert.equal(receivedMap[0].name, "fixed");
