...
/** Account-shaped names: real accounts are platform-owned (WP4); flagged here, resolved there. */
export const ACCOUNT_ENTITY_NAME = /^(?:app[-_ ]?user|user[-_ ]?profile|user[-_ ]?account|account|member(?:ship)?|user)s?$/i;
...
export function normalizeContractOwnership(contract, { buildProfile = null } = {}) {
  ...
  for (const entity of listOf(contract.entities)) {
    ...
    if (ACCOUNT_ENTITY_NAME.test(String(entity.name || ""))) accountCandidates.push(entity.name);
  ...
  for (const entity of accountCandidates) {
    platformRequirements.push({ type: "accounts", module: null, status: "unresolved", source: `entity:${entity}`,
      note: "account-shaped domain entity; real accounts are platform-owned (accounts module)" });
  }
  const roles = listOf(contract.auth?.roles);
  if (roles.length || signals.has("admin")) {
    platformRequirements.push({ type: "authorization", module: null, status: "unresolved",
      source: roles.length ? "auth.roles" : "signal:admin", roles });
  }
  if (signals.has("admin")) platformRequirements.push({ type: "admin", module: null, status: "unresolved", source: "signal:admin" });
