  ...
  const refreshDeterministicFoundation = (tree, spec) => {
    ...
    return composeScaffoldFoundation(
      composeCapabilityFoundation(refreshPlatformRuntime(tree), spec.capabilityGraph, {
        moduleLock: spec.moduleLock || null, identityPlan: spec.identityPlan || null, entitySchema: spec.entitySchema || null, routePlan: spec.routePlan || null, settingsPlan: spec.settingsPlan || null, behaviourPlan: spec.behaviourPlan || null, deliveryPlan: spec.deliveryPlan || null, insightPlan: spec.insightPlan || null,
      }).tree,
      spec.scaffoldGraph,
    ).tree;
  };
