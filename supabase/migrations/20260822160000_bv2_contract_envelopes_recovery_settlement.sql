...
alter table public.bv2_model_reservations
  ...
  add column recovery_policy_version text not null default 'legacy_v1'
    check (recovery_policy_version in ('legacy_v1','managed_recovery_v1')),
...
-- These rows pre-date activation of the managed-only recovery policy. Preserve their real lane and
-- settled provider evidence; the new columns only classify which funding policy produced them.
update public.bv2_model_reservations set
  funding_pool=case
    when usage_responsibility='thrallo_repair' and billing_lane='managed' then 'thrallo_recovery'
    else 'customer_generation'
  end,
  recovery_policy_version='legacy_v1';
...
-- The policy becomes active only when the restricted release records its immutable deployment
-- manifest. Applying this migration does not activate it and does not infer activation from a
-- migration timestamp. The existing feature-flag authority is already private and durable.
create or replace function public.activate_bv2_managed_recovery_policy(
...
begin
  if coalesce(p_deployment_commit,'') !~ '^[0-9a-f]{40}$'
    ...
    raise exception 'invalid Builder V2 recovery policy activation identity' using errcode='22023';
  end if;
  ...
  select * into v_existing from public.bv2_feature_flags
    where key='recovery.funding_policy' for update;
  if found then
    if coalesce(v_existing.value->>'state','')<>'active'
      or coalesce(v_existing.value->>'policyVersion','')<>'managed_recovery_v1'
      ...
      raise exception 'Builder V2 recovery policy activation identity changed' using errcode='23505';
    end if;
    return v_existing.value || jsonb_build_object('updatedBy',v_existing.updated_by);
  end if;
  v_value:=jsonb_build_object(
    'state','active',
    'policyVersion','managed_recovery_v1',
  ...
  insert into public.bv2_feature_flags(key,value,updated_at,updated_by)
    values('recovery.funding_policy',v_value,now(),trim(p_actor));
...
begin
  ...
  if new.recovery_policy_version='legacy_v1' then
    if v_active and v_starts_dispatch and new.usage_responsibility='thrallo_repair' then
  ...
  elsif new.recovery_policy_version='managed_recovery_v1' and v_starts_dispatch and not v_active then
    raise exception 'Builder V2 managed recovery policy is not activated' using errcode='P14P1';
  end if;
  return new;
end $$;

create trigger bv2_model_reservations_recovery_policy_guard
before insert or update of state,usage_responsibility,billing_lane,funding_pool,recovery_policy_version
on public.bv2_model_reservations
for each row execute function public.enforce_bv2_recovery_policy_activation();
...
begin
  ...
  select value into v_policy from public.bv2_feature_flags
    where key='recovery.funding_policy';
  if v_policy is null or v_policy->>'state'<>'active'
    or v_policy->>'policyVersion'<>'managed_recovery_v1'
    or coalesce(v_policy->>'deploymentCommit','') !~ '^[0-9a-f]{40}$'
    or coalesce(v_policy->>'deploymentManifestSha256','') !~ '^[0-9a-f]{64}$' then
    raise exception 'Builder V2 managed recovery policy is not activated' using errcode='P14P1';
  end if;
  ...
  if found then
    if v_existing.project_id<>p_project_id or v_existing.step<>p_step or v_existing.provider<>p_provider
      ...
      or v_existing.recovery_policy_version<>'managed_recovery_v1'
  ...
  if v_existing.id is not null then
    update public.bv2_model_reservations set state='held',released_at=null,
      ...
      recovery_policy_version='managed_recovery_v1',
...
revoke execute on function public.reserve_bv2_model_call_v4(uuid,uuid,uuid,text,text,text,text,text,text,text,numeric,numeric,numeric,timestamptz,bigint,jsonb)
  from public,anon,authenticated;
revoke execute on function public.activate_bv2_managed_recovery_policy(text,text,text)
  from public,anon,authenticated;
revoke execute on function public.enforce_bv2_recovery_policy_activation()
  from public,anon,authenticated;
...
grant execute on function public.reserve_bv2_model_call_v4(uuid,uuid,uuid,text,text,text,text,text,text,text,numeric,numeric,numeric,timestamptz,bigint,jsonb)
  to service_role;
grant execute on function public.activate_bv2_managed_recovery_policy(text,text,text) to service_role;
grant execute on function public.enforce_bv2_recovery_policy_activation() to service_role;
...
comment on table public.bv2_feature_flags is
  'Historical Builder rollout evidence plus the immutable service-role recovery funding policy activation authority.';
comment on column public.bv2_model_reservations.recovery_policy_version is
  'legacy_v1 preserves the funding behavior used by historical calls; managed_recovery_v1 is admitted only after deployment-manifest activation.';
